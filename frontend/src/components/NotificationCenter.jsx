            ...
            {notifications.length === 0 ? (
              ...
              <ul className="divide-y divide-gray-50">
                {notifications.map((notif) => (
                  <li
                    key={notif.id}
                    className={`p-4 hover:bg-gray-50 transition flex items-start space-x-3 ${notif.type === 'alert' ? 'bg-orange-50/50' : ''}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm ${notif.type === 'alert' ? 'font-medium text-gray-900' : 'text-gray-700'}`}>
                        {notif.message}
                      </p>
